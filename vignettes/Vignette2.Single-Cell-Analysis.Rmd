...
rid <- bfcquery(bfc, query = "vignette_2", "rname")$rid
...
if (length(rid) == 0) {
  ...
  urls <- pb_download_url(repo = "Muunraker/nipalsMCIA", tag = "latest")
  
  # you could add all of the urls to rid at once, but it would throw this error when it comes to downloading them: "Error in parse_url(url) : length(url) == 1 is not TRUE"
  for (url in urls) {
    # the rids are numbered in the order they are added e.g. BFC1, BFC2, ...
    message(paste("Downloading file", url))
    rid <- names(bfcadd(bfc, rname = "vignette_2", url))
...
# pipeline image
knitr::include_graphics(path = bfcquery(bfc, "Pipeline")$rpath)
```
...
path_metadata_sc <- bfcquery(bfc, query = "metadata_sc")$rpath

# MCIA results
# path_data_blocks <- bfcquery(bfc, query = "data_blocks")$rpath
path_mcia_results <- bfcquery(bfc, query = "mcia_results_sc")$rpath

# marker genes for cell type annotation with Seurat
path_marker_genes <- bfcquery(bfc, query = "marker_genes")$rpath

# the Seurat data's metric summary file from 10x Genomics
path_metrics_summary <- bfcquery(bfc, query = "metrics_summary")$rpath

# Seurat objects in different stages of processing
path_obj_raw <- bfcquery(bfc, query = "CITEseq_raw")$rpath
path_obj_annotated <- bfcquery(bfc, query = "CITEseq_annotated")$rpath
```
...
metadata_sc <- read.csv(file = path_metadata_sc, header = TRUE, row.names = 1)
...
mcia_results_sc <- readRDS(file = path_mcia_results)
mcia_results_sc
```
...
metrics_summary <- read.csv(file = path_metrics_summary)
metrics_summary
```
...
obj <- readRDS(file = path_obj_annotated)
...
markers_all <- read.csv(file = path_marker_genes)
