...
# files for running MCIA
pb_download(file = "metadata_sc.csv", # "Annotations"
            repo = "Muunraker/nipalsMCIA", tag = "v0.0.1", dest = path_inst)
pb_download(file = "data_blocks_sc.Rda", # "10x Genomics and Seurat"
            repo = "Muunraker/nipalsMCIA", tag = "v0.0.1", dest = path_inst)
pb_download(file = "data_sc_sce.Rda", # "Bioconductor"
            repo = "Muunraker/nipalsMCIA", tag = "v0.0.1", dest = path_inst)
...
# load the Bioconductor data (both MCIA formatted and the Seurat object) if desired
# load(file = file.path(path_inst, "data_sc_sce.Rda"))
```

```{r mcia-decomp-run, eval = FALSE}
# "largest_sv" results in a more balanced contribution from the blocks than the default "unit_var"
...
```{r mcia-plots-eigenvalue-scree, fig.dim = c(8, 6), echo = FALSE}
global_scores_eigenvalues_plot(mcia_results_sc)
```

#### Projection plot

```{r mcia-plots-projection, fig.dim = c(8, 8), echo = FALSE}
...
```{r mcia-plots-heatmap-global, fig.dim = c(8, 6), echo = FALSE}
...
```{r mcia-plots-heatmap-block, fig.dim = c(8, 6), echo = FALSE}
block_weights_heatmap(mcia_results_sc)
```

#### Loadings

```{r mcia-plots-heatmap-loadings, fig.dim = c(8, 6), echo = FALSE}
...
```{r mcia-plots-top-features-factor-2, fig.dim = c(10, 5)}
# define the loadings
all_abs_2 <- ord_loadings(mcia_out = mcia_results_sc, omic = "all",
                          absolute = TRUE, descending = TRUE, factor = 2)
prot_abs_2 <- ord_loadings(mcia_out = mcia_results_sc, omic = "adt",
                           absolute = TRUE, descending = TRUE, factor = 2)

# visualization
all_abs_2_vis <- vis_load_ord(gl_f_ord = all_abs_2, omic_name = "all",
                              colors_omics = colors_omics_sc)
prot_abs_2_vis <- vis_load_ord(gl_f_ord = prot_abs_2, omic_name = "adt",
                               colors_omics = colors_omics_sc)
...
# define the loadings
all_4 <- ord_loadings(mcia_results_sc, omic = "all", absolute = FALSE,
                      descending = TRUE, factor = 4)

# visualization
vis_load_ord(gl_f_ord = all_4, colors_omics = colors_omics_sc, n_feat = 60)
```
...
```{r seurat-qc-pre-filter, fig.dim = c(8, 5), echo = FALSE}
...
```{r seurat-qc-post-filter, fig.dim = c(8, 5), echo = FALSE}
...
```{r seurat-markers-dot-gex, fig.dim = c(20, 6), warning = FALSE, echo = FALSE}
p <- DotPlot(obj, features = unique(markers_all$Marker),
             cols = "RdBu", dot.scale = 3, cluster.idents = TRUE) 
...
DotPlot(obj, features = rownames(obj@assays[["ADT"]]@counts),
        cols = "RdBu", col.min = 0, dot.scale = 3, cluster.idents = TRUE) + 
  RotatedAxis()
...
```{r seurat-markers-violin-adt, fig.dim = c(16, 32), echo = FALSE}
...
# change colors from default if desired
...
```{r seurat-umap-adt, fig.dim = c(16, 16), echo = FALSE, eval = FALSE}
...
FeaturePlot(obj, features = c("CD3-TotalSeqB", "CD4-TotalSeqB",
            ...
            label = TRUE, label.size = 2, ncol = 2, raster = FALSE)
```
...
```{r seurat-check-annotations-t, fig.dim = c(16, 12), echo = FALSE}
VlnPlot(object = obj,
...
data_rna <- log2(data_rna + 1) # log-transform becuse the data is heavy tailed
...
data_adt <- log2(data_adt + 1) # log-transform becuse the data is heavy tailed
