...
![Vignette 2 Pipelines by Major Section](https://github.com/Muunraker/nipalsMCIA/releases/download/v0.99.5/Vignette-2-Pipeline.png)
...
mcia_results_sc_list <- readRDS(file = file.path(path_inst, "mcia_results_sc.Rds"))
names(mcia_results_sc_list)

# **** TEMPORARY - CONVERT TO NIPALS RESULT OBJECT DIRECTLY *********
mcia_results_sc <- new("NipalsResult",
                   global_scores = mcia_results_sc_list$global_scores,
                   global_loadings = mcia_results_sc_list$global_loadings,
                   block_score_weights = mcia_results_sc_list$block_score_weights,
                   block_scores = mcia_results_sc_list$block_scores,
                   block_loadings = mcia_results_sc_list$block_loadings,
                   eigvals = mcia_results_sc_list$eigvals,
                   col_preproc_method = mcia_results_sc_list$col_preproc_method,
                   block_preproc_method = mcia_results_sc_list$block_preproc_method,
                   block_variances = mcia_results_sc_list$block_variances,
                   metadata = mcia_results_sc_list$metadata)
```
...
You can also obtain the following bar plot using the output of `nipals_multiblock` e.g. with `ggplot(data.frame(table(mcia_results_sc@metadata)), aes(x = CellType, y = Freq, fill = CellType)) + geom_bar(stat = "identity")` (for the base plot).
