...
rid <- BiocFileCache::bfcquery(bfc, query = "single_cell_vignette", "rname")$rid
...
if (length(rid) == 0) {
  ...
  for (url in urls) {
    ...
    rid <- names(BiocFileCache::bfcadd(bfc, rname = "single_cell_vignette", url))
...
# pipeline image
knitr::include_graphics(path = BiocFileCache::bfcquery(bfc, "Single_Cell_Analysis_Vignette")$rpath)
```
