...
# note that you can also use the LabelClusters function to help label individual clusters
plot_seurat_clusters <- UMAPPlot(obj, label = TRUE, label.size = 4) +
...
```{r seurat-markers-dot-gex, fig.dim = c(20, 6), warning = FALSE}
...
```{r seurat-markers-feature-melanocytes, fig.dim = c(4, 4)}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-myocytes, fig.dim = c(4, 4)}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-platelets, fig.dim = c(4, 4)}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-t, fig.dim = c(16, 12)}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
                                      Cell_Type == "T"))$Marker,
            min.cutoff = 0, label = TRUE, label.size = 2,
