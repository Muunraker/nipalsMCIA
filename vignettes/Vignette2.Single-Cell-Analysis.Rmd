...
data_blocks_sc$mrna[1:5, 1:5]
data_blocks_sc$adt[1:5, 1:5]
```
...
```{r, fig.dim = c(4, 20)}
FeaturePlot(obj, features = unique(markers_all$Marker), label = TRUE, ncol = 4)
```
...
metadata_sc <- data.frame(obj$annotated_clusters) %>%
                rename("CellType" = "obj.annotated_clusters") %>%
                arrange(CellType)
