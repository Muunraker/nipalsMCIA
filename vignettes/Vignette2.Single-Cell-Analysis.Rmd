...
data_blocks_sc$mrna[1:5, 1:3]
data_blocks_sc$adt[1:5, 1:3]
```
...
mcia_results_sc <- nipals_multiblock(data_blocks_sc, preproc_method = "colprofile",
                                     num_PCs = 10, tol = 1e-12, plots = "none")
mcia_results_sc_none <- nipals_multiblock(data_blocks_sc, preproc_method = "none",
                                          num_PCs = 10, tol = 1e-12)

saveRDS(mcia_results_sc, file = file.path("..", "data", "mcia_results_sc.Rds"))
saveRDS(mcia_results_sc_none, file = file.path("..", "data", "mcia_results_sc_none.Rds"))
``` 

```{r}
mcia_results_sc <- readRDS(file = file.path("..", "data", "mcia_results_sc.Rds"))
mcia_results_sc_none <- readRDS(file = file.path("..", "data", "mcia_results_sc_none.Rds"))

names(mcia_results_sc)
```
### Visualizations

This data comes from only one subject, so we will use the previously defined cell types 
for the metadata.

### With Preprocessing

```{r, fig.dim = c(4, 4), fig.align = 'center'}
projection_plot(mcia_results_sc, "projection_global", orders = c(1, 2))
```
...
vis_load_plot(mcia_results_sc, axes = c(1, 4), colors_omics = colors_omics)
```

### Without Preprocessing

```{r, fig.dim = c(4, 4), fig.align = 'center'}
projection_plot(mcia_results_sc_none, "projection_global", orders = c(1, 2))
```

```{r, fig.dim = c(6, 4), fig.align = 'center'}
global_scores_heatmap(mcia_results_sc_none)
```

```{r, fig.dim = c(6, 4), fig.align = 'center'}
block_weights_heatmap(mcia_results_sc_none)
```

```{r, fig.dim = c(6, 4), fig.align = 'center'}
colors_omics <- get_colors(mcia_results_sc_none)
vis_load_plot(mcia_results_sc_none, axes = c(1, 4), colors_omics = colors_omics)
```
