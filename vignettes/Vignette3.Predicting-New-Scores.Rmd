...
metadata_input <- data.frame(metadata_NCI60[train_samples, ],
                                  row.names = rownames(data_blocks_train$mrna))
colnames(metadata_input) <- c("cancerType")
MCIA_train <- nmb_set_metadata(MCIA_train, metadata_input)
...
MCIA_out$cancerType <- metadata_input[, 1]
