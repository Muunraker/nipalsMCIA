...
MCIA_train$metadata<-data.frame(metadata_NCI60[train_samples,],row.names=rownames(data_blocks_train$mrna))
...
ggplot(data = MCIA_out_full, aes(x=Factor.1, y=Factor.2, color=cancerType, shape=set))+
  geom_point(size=3) + 
  theme_bw() +
  scale_color_manual(values=meta_colors)+
