...
# convert data_blocks to an MAE object
...
this data. To do so, the colData slot of the associated MAE object must be loaded
with sample-level metadata prior to invoking projection_plot(). The sample metadata
is composed of row names corresponding to the primary sample names of the MAE
object, and columns contain different metadata (e.g. age, disease status, etc.)
For instance, each of the 21 samples in the NCI-60 dataset represents a cell line 
with one of three cancer types: CNS, Leukemia, or Melanoma. We have provided this 
metadata as part of the `data(NCI60)` dataset and show how it can be included in 
the resulting MAE object using the `colData` parameter in `simple_mae()`,
or as `colData(data_blocks_mae)`:
...
We now rerun `nipals_multiblock()` using the updated MAE object, where the `colData`
is passed to the `metadata` slot of the `NipalsResult` instance,
...
# AK comment: we should only have one means of adding the metadata, which is via the colData column of the input data
# alternative method for adding metadata
# mcia_results <- nmb_set_metadata(mcia_results,metadata_NCI60)
```

The `color_col` argument of `projection_plot()` can then be used to determine
which column of `metadata` is used for coloring the individual data points, in this
case `cancerType`. `color_pal` can then be used with a vector of colors
(i.e. c('blue', 'red', 'green')) or `scales::<function>` to select a color palette.
Note: colors are   applied by lexicographically sorting the unique list of metadata
values, the first color is then assigned to the first value, second with second
and so on.
...
```
Using this plot one can observe that global factor scores for factor 1 and 2
can separate samples into their cancer types.

## Visualizing the Clustering of Samples by Factor Scores

A heatmap can be used to cluster the samples based on global scores across all factors
...
Here, we can observe that factors 7-9 have the strongest contribution
...
Using the graph above we can deduce that individual miRNA features tend to
spread out much more from the (0,0) region than mRNA and prot features,
therefore, per miRNA feature, there is a stronger contribution of these
variables to the decomposition via factor 1 and 4.
...
Here, we return and visualize two ranked lists for Factor 1: using all omics,
or just mrna.
...
By using the graph on the left we see that miRNA is playing the strongest
role across all positive feature loadings whereas the right graph allows us to
focus on mRNA signals where genes like *GNS* and *WASL* come up as top
contributors.
...
values according to the absolute value. Doing so allowed us to identify 9 (out
of 15) features with a strong negative loading value. Even more impressive is the 
right graph where only prot features are plotted using absolute ranking. In this
second plot only strong negative loading values are present showcasing the
importance of using absolute rankings, alternatively, you can make two scree
plots with no absolute ranking and then apply descending order for one and
ascending order for the other.
...
# visualization
all_4_plot <- vis_load_ord(gl_f_ord = all_4, omic_name='all', 
                           colors_omics = colors_omics, n_feat = 60)
all_4_plot
```
...
factor3_paths[,"padj"] <- sapply(factor3_paths[,"padj"], to_sci)
...
factor3_paths[,"pathway"] <- apply(X=factor3_paths[,"pathway"], MARGIN=1, FUN =clean_pathway_name)
```
