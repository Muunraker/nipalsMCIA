...
`global_scores` refers to $f^{(j)}$'s, `global_loadings` to $a^{(j)}$'s, `block_score_weights` to $G$, `block_scores` to $f_k^{(j)}$'s, `block_loadings` $a_k^{(j)}$'s, `eigvals` to $e$, `preproc_method` to the pre-processing method
