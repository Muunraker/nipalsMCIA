...
The `nipals_multiblock` function implements performs MCIA using the NIPALS algorithm.  
`nipals_multiblock` outputs results of the decomposition, including a low-dimensional 
embedding of the data in its global scores, and the contributions of the data 
blocks (block score weights) and features (global loadings) to the global scores.  
...
have a list of data blocks with matching sample names (with optional sample-level
metadata), we provide a simpleconversion function (`simple_mae.R`) to generate an 
MAE object from the data blocks.  For more sophisticated MAE object construction,
...
this data. The `projection_plot()` function can incorporate sample metadata if 
it is included in the `colData` slot of the associated MAE object.  The sample
metadata contains rownames corresponding to the primary sample names of the MAE
...
We now rerun `nipals_multiblock()` using the updated MAE object, the `colData`
