...
(MAE) object. Such objects represent a sophisticated approach to organizing multi-omics
data in which each assay can be stored as an individual experiment alongside any
relevant metadata for the experiment as a whole. *The function will automatically
detect whether your data is in the form of a list or a MAE.*  (Note that sample metadata
must be supplied as a dataframe in the colData attribute of the MAE object).
...
```{r part-1-mcia, warning = FALSE, message = FALSE, fig.dim = c(7, 4), fig.align = 'center'}
set.seed(42)
...
```{r part-1-visualize-clusters, fig.dim = c(4, 4), fig.align = 'center'}
...
```{r part-1-visualize-clusters-color-col, fig.dim = c(4, 4), fig.align = 'center'}
...
```{r part-1-global-heatmap, fig.dim = c(6, 4), fig.align = 'center'}
...
```{r part-1-global-heatmap-colored, fig.dim = c(6, 4), fig.align = 'center'}
global_scores_heatmap(mcia_results,
                      color_col = "cancerType", color_pal = meta_colors)
```
...
```{r part-2-block-heatmap, fig.dim = c(4, 2.5), fig.align = 'center'}
...
```{r part-2-loadings,fig.align='center',fig.dim=c(6,4)}
...
```{r part-2-factor-1, fig.dim = c(8, 3), fig.align='center'}
# define the loadings
...
```{r part-2-factor-2, fig.dim = c(8, 3),fig.align='center'}
# define the loadings
...
```{r part-2-factor-4, fig.dim = c(10, 4),fig.align='center'}
# define the loadings
