...
omic_list<-omic_list<-c("mrna","miRNA","prot")

# create a dataframe attributing specific colors to each color in omic_list
omic_colors<-function(omic_list){
   color_dict<-data.frame(brewer.pal(n=length(omic_list), name="Set1"))
   rownames(color_dict)<-omic_list
   colnames(color_dict)<-c('color')
   return(color_dict)
}
...
vis_load_ord<-function(gl_f_ord, omic_list, n_feat=30){
   colors_omic<-omic_colors(omic_list)
   n_plot<-min(nrow(gl_f_ord),n_feat)
   omic_subset<-names(table(droplevels(gl_f_ord[0:n_plot,])$omic))
   color_vals<-colors_omic[omic_subset,'color']
   ggplot(gl_f_ord[1:n_plot,], aes(x=factor(omic_name,level=omic_name), y=loading,color=omic))+
         geom_point()+
         theme_bw()+
         xlab('Feature')+
...
vis_load_plot<-function(mcia_out,axes=c(1,2),omic_list){
   ...
   colors_omic<-data.frame(brewer.pal(n=length(omic_list), name="Set1"))
   rownames(colors_omic)<-omic_list
   colnames(colors_omic)<-c('color')
   
   colnames(gl_f)<-c(paste0('Axis_',axes[1]),paste0('Axis_',axes[2]),"omic")
   color_vals<-omic_colors(omic_list)$color
   out<-ggplot(gl_f, aes_string(x=colnames(gl_f)[1], y=colnames(gl_f)[2], color="omic"))+
...
all_pos_1<-ord_loadings(mcia_results, omic="all",absolute=FALSE,pos=TRUE,factor=1)
mrna_pos_1<-ord_loadings(mcia_results, omic="mrna",absolute=FALSE,pos=TRUE,factor=1)
all_abs_2<-ord_loadings(mcia_results, omic="all",absolute=TRUE,pos=TRUE,factor=2)
prot_abs_2<-ord_loadings(mcia_results, omic="prot",absolute=TRUE,pos=TRUE,factor=2)

all_pos_1_vis<-vis_load_ord(all_pos_1,omic_list)
mrna_pos_1_vis<-vis_load_ord(mrna_pos_1,omic_list)
all_abs_2_vis<-vis_load_ord(all_abs_2,omic_list)
prot_abs_2_vis<-vis_load_ord(prot_abs_2,omic_list)
...
```
```{r}
all_abs_5<-ord_loadings(mcia_results, omic="all",absolute=FALSE,pos=TRUE,factor=4)
all_abs_5_vis<-vis_load_ord(all_abs_5,n_feat=60,omic_list)
all_abs_5_vis
```
