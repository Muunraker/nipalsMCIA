...
#' @param pl display plot (pl=TRUE) inline
#' @examples
#' vis_load_ord(all_pos_1_df,colors_omics=colors_omics, pl=TRUE)
#' all_pos_1_vis<-vis_load_ord(all_pos_1_df,colors_omics=colors_omics, pl=FALSE)

vis_load_ord <- function(gl_f_ord, colors_omics, n_feat = 15, pl=TRUE) {
...
all_pos_1_vis <- vis_load_ord(gl_f_ord = all_pos_1, colors_omics=colors_omics,pl=FALSE)
mrna_pos_1_vis <- vis_load_ord(gl_f_ord = mrna_pos_1,colors_omics=colors_omics,pl=FALSE)

ggarrange(all_pos_1_vis, mrna_pos_1_vis)
```

```{r, fig.width=10, fig.height=5}
all_abs_2_vis <- vis_load_ord(gl_f_ord = all_abs_2, colors_omics=colors_omics, pl=FALSE)
prot_abs_2_vis <- vis_load_ord(gl_f_ord = prot_abs_2, colors_omics=colors_omics, pl=FALSE)

ggarrange(all_abs_2_vis, prot_abs_2_vis)
```

```{r, fig.width=10, fig.height=4}
all_4 <- ord_loadings(mcia_results, omic = "all", 
                          absolute = FALSE, descending = TRUE, factor = 4)
all_4_plot<-vis_load_ord(gl_f_ord = all_4, colors_omics=colors_omics, n_feat = 60,pl=TRUE)
```
