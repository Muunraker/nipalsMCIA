...
The `coloring` argument of `projection_plot()` can then be used to determine which column of `metadata` is used for coloring the projection plots, in this case the column is `cancerType`:
```{r Visualize Clusters Coloring, fig.height=4, fig.width=4, fig.align='center'}
projection_plot(mcia_results, "projection_global", orders = c(1,2),
                coloring = "cancerType", legend_loc = "bottomleft")
```
