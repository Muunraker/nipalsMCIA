...
data_blocks_sc_sce_seurat <-
  AddMetaData(object = data_blocks_sc_sce_seurat,
              metadata = as.data.frame(rowData(tenx_pbmc3k),
                                       row.names = rownames(tenx_pbmc3k)))
# rownames(data_blocks_sc_sce_seurat[[]]) <- rownames(data_blocks_sc_sce_seurat)
...
(metrics_summary <- read.csv(
    file = file.path(path_data, "tenx_pbmc5k_CITEseq",
...
p <- DotPlot(object = obj, features = unique(markers_all$Marker),
...
```{r seurat-markers-feature-b, fig.dim = c(16, 24), dpi = 42}
...
```{r seurat-markers-feature-mdcs, fig.dim = c(16, 8), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-melanocytes, fig.dim = c(16, 4), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-myocytes, fig.dim = c(16, 4), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-nk, fig.dim = c(16, 12), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
                                      Cell_Type == "NK"))$Marker,
            min.cutoff = 0, label = TRUE, label.size = 2,
...
```{r seurat-markers-feature-pdcs, fig.dim = c(16, 12), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-plasma, fig.dim = c(16, 12), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-platelets, fig.dim = c(12, 4), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
...
```{r seurat-markers-feature-t, fig.dim = c(16, 16), dpi = 42}
FeaturePlot(object = obj,
            features = (dplyr::filter(markers_all,
                                      Cell_Type == "T"))$Marker,
            min.cutoff = 0, label = TRUE, label.size = 2,
...
```{r seurat-markers-violin-adt, fig.dim = c(16, 32), dpi = 48}
VlnPlot(object = obj, features = rownames(obj[["ADT"]]),
...
```{r seurat-umap-both, fig.dim = c(16, 8), dpi = 48}
# change colors and themes as desired
plot_annotated_clusters <- UMAPPlot(object = obj, label = TRUE, label.size = 4,
                                    cols = meta_colors_sc) +
...
```{r seurat-umap-adt, fig.dim = c(16, 16), dpi = 48}
# plot ADT information on top as an example

DefaultAssay(object = obj) <- "ADT"

FeaturePlot(object = obj, features = c("CD3-TotalSeqB", "CD4-TotalSeqB",
...
```{r seurat-check-annotations-t, fig.dim = c(16, 12), dpi = 48}
VlnPlot(object = obj,
